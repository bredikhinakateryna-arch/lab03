...
public class RBTreeLeaderboard : ILeaderboard
{
    ...
    {
        if (!_playerLookup.TryGetValue(summonerName, out var playerToUpdate))
        {
            return; 
        }

        _tree.Delete(playerToUpdate);

        playerToUpdate.UpdateMMR(newMMR);

        _tree.Insert(playerToUpdate);
    }
